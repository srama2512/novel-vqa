...
def prepro_question(imgs, params):
    ...
    for i,img in enumerate(imgs):
        ...
        if params['token_method'] == 'nltk':
            ...
            #with open('outCheck.txt', 'a') as outFile:
            #    print(s)
            #    outFile.write(s+'\n')
            #    for a1 in word_tokenize(str(s).lower()): 
            #        outFile.write(a1+' ')
            #    outFile.write('\n')

            #txt = word_tokenize(str(s).lower())
            txt = prepro_sentence(s)#word_tokenize(str(s).lower())
...
def build_vocab_question(imgs, params):
    ...
    if params['extern_vocab'] == '':
        ...
        vocab = json.load(open(params['extern_vocab']))
        vocab_set = set(vocab)
        print('inserting the special UNK token') 
        for img in imgs:
            txt = img['processed_tokens']
            question = [w if w in vocab_set else 'UNK' for w in txt]
            img['final_question'] = question
...
def encode_answer(imgs, atoi):
    ...
    for i, img in enumerate(imgs):
        # in case of external vocab, answer may not be present in atoi
        # put a dummy token, the 'ans' does not matter in this case
        if img['ans'] in atoi:
...
def main(params):
    ...
    if params['num_val'] > 0:
        ...
        imgs_val = imgs_train[-(num_val):]
        imgs_train = imgs_train[0:-(num_val-1)]
    ...
    print('Number of train: %d'%(len(imgs_train)))
    print('Number of val  : %d'%(len(imgs_val)))
    print('Number of test : %d'%(len(imgs_test)))
    ...
    A_val = encode_answer(imgs_val, atoi)
    ...
    f.create_dataset("answers_val", dtype='uint32', data=A_val)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_val', default=0, type=int, help='number of val images to use from the training set')
    ...
    parser.add_argument('--extern_vocab', default='../vocabs/vocab_oracle.json', help='external vocabulary in case of new model, change to train / general depending on use case')
    parser.add_argument('--extern_ans_vocab', default = '', help='external answer vocabulary in case of new model')
    parser.add_argument('--save_vocab', default =0, type=int, help='save current vocabulary?')
    parser.add_argument('--vocab_save_path', default='vocab.json', help='save path for current vocabulary')
    parser.add_argument('--ans_vocab_save_path', default='ans.json', help='save path for the current answer encoding')
    args = parser.parse_args()
    params = vars(args) # convert to ordinary dict
    print 'parsed input parameters:'
    print json.dumps(params, indent = 2)
    
    vocab, top_ans = main(params)

    if params['save_vocab'] == 1:
        json.dump(vocab, open(params['vocab_save_path'], 'w'))
        json.dump(top_ans, open(params['ans_vocab_save_path'], 'w'))
