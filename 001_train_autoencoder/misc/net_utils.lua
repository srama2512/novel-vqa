...
function net_utils.build_cnn(cnn, opt)
  ...
  cnn_part:add(nn.L2Normalize())
  local temp = nn.Linear(4096,encoding_size)
  temp.weight:uniform(-0.08, 0.08)
  temp.bias:fill(0)
  cnn_part:add(temp)
  return cnn_part
end

function net_utils.build_cnn_2(cnn, opt)
  local layer_num = utils.getopt(opt, 'layer_num', 38)
  local backend = utils.getopt(opt, 'backend', 'cudnn')
  local encoding_size = utils.getopt(opt, 'encoding_size', 512)
...
function net_utils.build_cnn_inception(cnn, opt)
  ...
  cnn_part:add(nn.L2Normalize())
  local temp = nn.Linear(2048,encoding_size)
  temp.weight:uniform(-0.08, 0.08)
  temp.bias:fill(0)
  cnn_part:add(temp)
  return cnn_part
...
function net_utils.build_cnn_inception_2(cnn, opt)
  local layer_num = utils.getopt(opt, 'layer_num', 30)
  local backend = utils.getopt(opt, 'backend', 'cudnn')
  local encoding_size = utils.getopt(opt, 'encoding_size', 512)
...
function net_utils.prepro_inception(imgs, data_augment, on_gpu)
  assert(data_augment ~= nil, 'pass this in. careful here.')
  assert(on_gpu ~= nil, 'pass this in. careful here.')
