...
cmd:option('-max_iters', 22501, 'max number of iterations to run for (-1 = run forever)')
...
cmd:option('-save_model_every', 22500, 'saving a model every half epoch')
...
local function lossFun()
  ...
  local flag = 0
  if math.random() <= opt.rand_val then
	  sentInput:fill(0)
	  flag = 1
  end
...
while true do  
  ...
  local cnn_params_norm = torch.norm(protos.cnn:get(40).weight)
