-- Code to train an autoencoder based on text data for architecture 1
...
cmd:option('-num_layers', 1, 'number of layers in lstm')

-- Optimization: General
cmd:option('-max_iters', 75001, 'max number of iterations to run for (-1 = run forever)')
...
cmd:option('-save_model_every', 75000, 'saving model every half epoch')
...
if string.len(opt.start_from) > 0 then
  ...
  aeOpt.num_layers = opt.num_layers
