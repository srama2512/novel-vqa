-- Code to train AutoEncoder model on weak paired text and image data for architecture 1
...
cmd:option('-lstm_average_path', '', 'path to h5 file containing average lstm sentence vector')
cmd:option('-img_average_path', '', 'path to h5 file containing average image vector')
...
cmd:option('-batch_size',16,'what is the batch size in number of sentences per batch?')
...
-------------------------------------------------------------------------------
-- Load the average lstm sentence vector
-------------------------------------------------------------------------------
local h5_file = hdf5.open(opt.lstm_average_path, 'r')
local lstm_average_vector = h5_file:read('/mean_vector'):all()
h5_file:close()
local h5_file = hdf5.open(opt.img_average_path, 'r')
local img_average_vector = h5_file:read('/mean_vector'):all()
h5_file:close()

-- Normalize the cnn feature vector
local nm = torch.sqrt(torch.sum(torch.cmul(img_average_vector, img_average_vector), 2)) 
img_average_vector:div(nm[1][1])
--
-------------------------------------------------------------------------------
...
if string.len(opt.start_from) > 0 then
  ...
  aeOpt.nhimage = opt.nhimage
  protos.ae = nn.AutoEncoder_vqa(aeOpt)

  local ae_temp= torch.load(opt.start_from_text).protos.ae
  local ae_modules = ae_temp:getModulesList()
  for k,v in pairs(ae_modules) do net_utils.unsanitize_gradients(v) end
  
  protos.ae.encoder = ae_temp.encoder:clone()
  protos.ae.decoder = ae_temp.decoder:clone()
  protos.ae.multimodal_net = ae_temp.multimodal_net:clone() 
  --print(protos.ae.multimodal_net:get(1).forwardnodes[11].data.module)
  --protos.ae.multimodal_net:get(1).forwardnodes[11].data.module.weight:uniform(-0.0008, 0.0008)
  protos.ae.lookup_table = ae_temp.lookup_table:clone()
  ...
  protos.cnn = net_utils.build_cnn_2(cnn_raw, {encoding_size = opt.input_encoding_size, backend = cnn_backend})
  cnn_raw = nil
end
...
thin_ae.multimodal_net:share(protos.ae.multimodal_net, 'weight', 'bias')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    data.imgs = net_utils.prepro(data.imgs, false, opt.gpuid >= 0) -- preprocess in place, and don't augment
	...
	local logprobs = protos.ae:forward{feats, data.labels, data.labels, 0}
    local loss = protos.crit:forward(logprobs, data.labels)
    loss_sum = loss_sum + loss
    loss_evals = loss_evals + 1

    -- forward the model to also get generated samples for each sentence
	local seq = protos.ae:sample(feats, data.labels)
...
local function lossFun()
  ...
  data.imgs = net_utils.prepro(data.imgs, true, opt.gpuid >= 0) -- preprocess in place, do data augmentation

  local encoder_skip_flag = 0
  local sentInput

  if math.random() <= 0.5 then
      sentInput = torch.repeatTensor(lstm_average_vector, opt.batch_size, 1) 
    encoder_skip_flag = 1
  else
	  sentInput = data.labels:clone()
  end
  
  if opt.gpuid >= 0 then
	  data.imgs = data.imgs:cuda()
	  data.labels = data.labels:cuda()
	  sentInput = sentInput:cuda()
  end
  ...
  local logprobs = protos.ae:forward{feats, sentInput, data.labels, encoder_skip_flag}
  ...
  local dfeats, ddummy = unpack(protos.ae:backward({feats, sentInput, data.labels, encoder_skip_flag}, dlogprobs))
...
while true do  
  ...
  local cnn_params_norm = torch.norm(cnn_params) 
  local num_updates_cnn = torch.sum(torch.ge(torch.abs(torch.mul(cnn_grad_params, opt.cnn_learning_rate)), torch.mul(torch.abs(cnn_params), 0.01)))
