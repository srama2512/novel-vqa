...
def prepro_sentence(sents):
    ...
    for sentIdx in xrange(len(sents)):
        sents_prepro.append(''.join([l for l in sents[sentIdx] if ord(l) < 128]).encode('utf-8').lower().translate(None, string.punctuation).strip().split())
        if sentIdx%1000 == 0:
            sys.stdout.write('Number of words processed: %7d/%7d\r'%(sentIdx, totalLen))
            sys.stdout.flush()
    return sents_prepro
...
def create_vocab(dataset, params):
    ...
    if params['ext_vocab'] == '': 
        ...
        vocab_set = set(vocab)
        for sent in dataset['tokenized']:
            sent_final = [w if w in vocab_set else 'UNK' for w in sent]
...
def encode_sentences_less_memory(dataset, params, wtoi, split):
    ...
    L = {}
    L[split] = np.zeros((count_curr, max_length), dtype='uint32')
    input_length[split] = np.zeros((count_curr), dtype='uint32')
    ...
    for sentIdx in range(len(dataset['final'])):
        ...
        if splitCurr == split:
            for k, w in enumerate(sent):
                if k < max_length:
                    #Ltemp[0, k] = wtoi[w]
                    L[split][count_curr, k] = wtoi[w]
            #input_sentence[splitCurr].append(Ltemp)
            #lengthTemp[0] = min(max_length, len(sent)) 
            input_length[splitCurr][count_curr] = min(max_length, len(sent))
            count_curr += 1
            #input_length[splitCurr].append(lengthTemp)

    #L[split] = np.concatenate(input_sentence[split], axis=0)
    #input_length[split] = np.concatenate(input_length[split], axis=0)
...
if not os.path.isfile('preprocessed_general/BookCorpus.json'):
    ...
    startTime = time.time()
    BookCorpusDataset['tokenized'] = prepro_sentence(BookCorpusDataset['unprocessed'])
    json.dump(BookCorpusDataset, open('preprocessed_general/BookCorpus.json', 'w'))

    print('Finished preprocessing: %.3f secs'%(time.time()-startTime))
else:
    print('Already preprocessed BookCorpus and Wiki')
    startTime = time.time()
    BookCorpusDataset = json.load(open('preprocessed_general/BookCorpus.json'))
...
del BookCorpusDataset['tokenized']
print('Deleted tokenized dataset')

L, input_length = encode_sentences_less_memory(BookCorpusDataset, params, wtoi, 'train')
...
L, input_length = encode_sentences_less_memory(BookCorpusDataset, params, wtoi, 'val')

f.create_dataset("labels/val", dtype='uint32', data=L['val'])
f.create_dataset("label_length/val", dtype='uint32', data=input_length['val'])

L, input_length = encode_sentences_less_memory(BookCorpusDataset, params, wtoi, 'test')
...
out['num_train'] = len(BookCorpusDataset['final'])-params['num_test']-params['num_val']
