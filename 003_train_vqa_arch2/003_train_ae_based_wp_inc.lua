-- Code to finetune multimodal autoencoder architecture 2 trained on weak paired data on VQA data
...
cmd:option('-start_from', '', 'starting from this model')
...
cmd:option('-nhimage', 4096, 'image feature vector dimensions')
--check point
...
cnn_projection = nn.Sequential():add(modelT.cnn:get(32):clone())
print(cnn_projection)
net_utils.unsanitize_gradients(cnn_projection)
...
if string.len(opt.start_from) > 0 then
	loadedModel = torch.load(opt.start_from)
	cnn_w:copy(loadedModel['cnn_w'])
	encoder_w_q:copy(loadedModel['encoder_w_q'])
	multimodal_w:copy(loadedModel['multimodal_w'])
end
...
torch.save(string.format(model_path..'lstm.t7',i),{cnn_w=cnn_w, encoder_w_q=encoder_w_q, multimodal_w=multimodal_w}) 
--]]
