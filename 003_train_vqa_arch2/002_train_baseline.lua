-- Code to train VQA baseline model for architecture 2
...
cmd:option('-input_encoding_size', 512, 'size of word representation')
cmd:option('-rnn_size', 512, 'size of the rnn hidden layer')
cmd:option('-num_layers', 1, 'number of hidden layers in RNN')
...
multimodal_net=nn.Sequential()
				:add(nn.Dropout(0.5))
				:add(nn.Linear(opt.rnn_size,noutput))

cnn_projection = nn.Sequential():add(nn.Linear(nhimage, opt.input_encoding_size))
