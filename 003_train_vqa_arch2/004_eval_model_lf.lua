...
cmd:text('Options')
-- Data input settings
cmd:option('-input_img_h5_1','data_img.h5','path to the h5file containing the first model image feature')
cmd:option('-input_img_h5_2','data_img.h5','path to the h5file containing the second model image feature')
...
cmd:option('-model_path_1', 'model/lstm.t7', 'path to first model in ensemble')
cmd:option('-model_path_2', 'model/lstm.t7', 'path to second model in ensemble')
cmd:option('-input_ae_model', 'model/lstm.t7', 'path to the input base autoencoder model used')
...
cmd:option('-nhimage_1', 4096, 'Image vector size for model 1')
cmd:option('-nhimage_2', 2048, 'Image vector size for model 2')
...
------------------------------------------------------------------------
print('DataLoader loading h5 file: ', opt.input_img_h5_1)
local h5_file = hdf5.open(opt.input_img_h5_1, 'r')
...
------------------------------------------------------------------------
local model_path_ae = opt.input_ae_model
local batch_size=opt.batch_size
local modelT = torch.load(model_path_ae).protos

local model_path = opt.model_path_1
local batch_size=opt.batch_size
local embedding_size_q=opt.input_encoding_size
local lstm_size_q=opt.rnn_size
local nlstm_layers_q=opt.rnn_layer
local nhimage=opt.nhimage_1
...
cnn_projection = nn.Sequential():add(nn.Linear(opt.nhimage_1, EncOpt.input_encoding_size))
...
model_loaded = torch.load(opt.model_path_1)
...
local scores_1, qids = evaluateModel()

------------------------------------------------------------------------
-- Loading image features
------------------------------------------------------------------------
print('DataLoader loading h5 file: ', opt.input_img_h5_2)
local h5_file = hdf5.open(opt.input_img_h5_2, 'r')
dataset['fv_im'] = h5_file:read('/images_test'):all()
h5_file:close()
...
local model_path = opt.model_path_2
local batch_size=opt.batch_size
local embedding_size_q=opt.input_encoding_size
local lstm_size_q=opt.rnn_size
local nlstm_layers_q=opt.rnn_layer
local nhimage=opt.nhimage_2
local common_embedding_size=opt.common_embedding_size
local noutput=opt.num_output
local dummy_output_size=1
...
cnn_projection = nn.Sequential():add(nn.Linear(opt.nhimage_2, EncOpt.input_encoding_size))
...
model_loaded = torch.load(opt.model_path_2)
encoder_w_q,encoder_dw_q=encoder_model:getParameters()
encoder_w_q:copy(model_loaded.encoder_w_q)
multimodal_w,multimodal_dw=multimodal_net:getParameters()
multimodal_w:copy(model_loaded.multimodal_w)
...
local scores_2, qids = evaluateModel()
local scores = scores_1:clone():fill(0)
scores:add(0.5, scores_1)
scores:add(0.5, scores_2)
