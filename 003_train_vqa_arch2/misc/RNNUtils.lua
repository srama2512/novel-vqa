...
if opt.gpuid >= 0 then
	function encoding_onehot(batch_word,batch_length,vocabulary_size)
		--batch_word: batch_size x MAX_LENGTH matrix
		...
		local D=batch_word:size(2);
		local L=batch_length_sorted[1];
		local batch_word_t=batch_word:index(1,sort_index):cuda():t()[{{1,L}}];
...
function rnn_forward(net_buffer,init_state,inputs,sizes)
	...
	for i=1,N do
		...
		elseif sizes[i]>sizes[i-1] then
			--right align
			--print('\ni value: ' .. i)
			--print('Sizes of init_state: ', init_state:size())
			--print('Current batch_size: ' .. sizes[i])
			--print('--------------------------------')
