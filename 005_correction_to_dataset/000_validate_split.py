...
for n_plu, n in bar(zip(novel_plu, novel)):
	...
	f.write('%20s%20s%20s%20s\n' % (n, n_plu, cnt_noplu, cnt_plu))

with open('log_mine_json_dump.json', 'w') as outfile:
    json.dump({'plu' : all_plu, 'noplu' : all_noplu}, outfile)
...
f.write('num_total %d\nnum_aff %d\nnum_aff_noplu %d\n num_sent_total %d\n num_sent_plu %d\nnum_sent_noplu %d\n' % (num_total, num_aff, num_aff_noplu, num_sent_total, num_sent_plu, num_sent_noplu))
f.close()
